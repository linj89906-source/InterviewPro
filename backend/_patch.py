...
ACCOMMODATION_TYPES = {
    ...
    # 全部住宿
    "all": "060000|060100|060200|060300|060400|060500|100000|100100",
}
...
def filter_accommodation_pois(pois: list) -> list:
    ...
    filtered = []
    for p in pois:
        code = (p.get("typecode", "") or "")[:2]
        if code == "06" and (p.get("typecode", "") or "")[:4] != "0600":
            filtered.append(p)
    kept = len(filtered)
    total = len(pois)
    if kept < total:
        import logging
        logger = logging.getLogger("app.services.map_service")
        logger.info("POI filter: kept %d/%d", kept, total)
    return filtered
    ...
    for cat, result in zip(categories, results):
        ...
        try:
            ...
            for p in pois:
                ...
                p["_cost_hint"] = ACCOMMODATION_COST_HINT.get(
